...
export async function POST(request: NextRequest) {
  try {
    ...
    const pv = await prisma.promptVersion.create({
      data: { projectId, version: ver, userInput: JSON.stringify(designSpec.object), craftedPrompt: result.craftedPrompt, negativePrompt: result.negativePrompt, styleNotes: result.content.slice(0, 200), clarityScore: 0.8, isApproved: false },
    });
